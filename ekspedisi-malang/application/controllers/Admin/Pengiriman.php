...
class Pengiriman extends CI_Controller {
	...
	public function insert()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			$data['sopir'] = $this->Pengiriman_model->get_sopir();
	...
	public function detail($id)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['transaksi'] = $this->Pengiriman_model->get_transaksi($id);
			$this->load->view('admin/header');
			$this->load->view('admin/pengiriman/detail',$data);
			$this->load->view('admin/footer');
		} else {
			$this->Pengiriman_model->insert_detail($id,$this->input->post('fk_transaksi'));
